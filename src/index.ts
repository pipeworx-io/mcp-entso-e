...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'ENTSO-E Transparency Platform');
}
...
function parseEntsoeXml(xml: string, query: Record<string, string>) {
  // Strip namespace prefixes so bare-tag regexes work regardless of xmlns declarations
  const stripped = xml.replace(/<([a-zA-Z][a-zA-Z0-9]*):/g, '<').replace(/<\/([a-zA-Z][a-zA-Z0-9]*):/g, '</');
  const tsBlocks = [...stripped.matchAll(/<TimeSeries>([\s\S]*?)<\/TimeSeries>/g)].map((m) => m[1]);
